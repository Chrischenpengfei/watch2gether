...
def showtime(bot,update):
    ...
    data_store(inputRoomName,inputCategory,inputShowTime,room_url,creationTime)

    return ConversationHandler.END

def data_store(inputRoomName,inputCategory,inputShowTime,room_url,roomCreationTime):
    try:
        conn = sqlite3.connect("watch2gether.db")
        cursor = conn.cursor()
        cursor.execute("INSERT INTO TB_ROOM(ROOMNAME,CODE_CATEGORY,SHOWTIME,URL,CREATETIME) VALUES (?,?,?,?,?);",(inputRoomName,inputCategory,inputShowTime,room_url,roomCreationTime))
        conn.commit()
        conn.close()
    except Exception as e:
        print(e)
...
def view_category(bot,update):
    ...
    try:
        ...
        cursor.execute('SELECT * FROM TB_ROOM WHERE CODE_CATEGORY=?',(inputCategory,))
        data = cursor.fetchall()

        if len(data) != 0:
            for row in cursor.execute('SELECT * FROM TB_ROOM WHERE CODE_CATEGORY=?',(inputCategory,)):
                text+='Room    : <b>'+row[1]+'</b>\n'+'Category: <b>'+categoryDict.get(row[2])+'</b>\n'+'Showtime: <b>'+row[3][:-10]+'</b>\n'+'RoomURL : <a>'+row[4]+'</a>\n'
                text+='\n'
        else:
            text += 'There are no room under this catagory, try other catagory or create one yourself maybe? :)'
    except Exception as e:
        print(e)
...
def CleanOutdatedRoom():
    while True:
        try:
            conn = sqlite3.connect("watch2gether.db")
            cursor = conn.cursor()
            time_now = datetime.datetime.now()
            for row in cursor.execute("SELECT CREATETIME FROM TB_ROOM;"):
                create_time, = row
                if (time_now - datetime.datetime.strptime(create_time,'%Y-%m-%d %H:%M:%S.%f')) > datetime.timedelta(days = 1): 
                    cursor.execute("DELETE FROM TB_ROOM WHERE CREATETIME = ?",(create_time,))
            conn.commit()
            time.sleep(5)
...
def main():
    ...
    clean_outdate_rm = threading.Thread(target = CleanOutdatedRoom)
    clean_outdate_rm.daemon = True

    clean_outdate_rm.start()
